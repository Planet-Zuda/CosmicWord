...
if ( ! function_exists( 'wp_redirect' ) ) :
	 ...
	 */
function wp_redirect( $location, $status = 302, $x_redirect_by = 'CosmicWord' ) {
	global $is_IIS;

	// Apply the wp_redirect filter to allow modifications to the URL.
	$location = apply_filters( 'wp_redirect', $location, $status );

	// Apply the wp_redirect_status filter to allow modifications to the status code.
	$status = apply_filters( 'wp_redirect_status', $status, $location );

	// If no location is provided, exit early.
	if ( ! $location ) {
		return false;
	}

	// Ensure the status code is valid for redirection.
	...
	// Apply the x_redirect_by filter.
