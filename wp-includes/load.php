...
function wp_fix_server_vars() {
    ...
    // Define allowed server values with strict types
    ...
    foreach (['REQUEST_URI', 'SCRIPT_NAME', 'PATH_INFO', 'PHP_SELF', 'SCRIPT_FILENAME', 'PATH_TRANSLATED'] as $key) {
        if (isset($_SERVER[$key])) {
            ...
            $_SERVER[$key] = preg_replace('/[\x00-\x1F\x7F]/', '', $_SERVER[$key]);
            // Normalize directory separators
            $_SERVER[$key] = str_replace(['\\', '//'], '/', $_SERVER[$key]);
            // Remove multiple consecutive slashes
            $_SERVER[$key] = preg_replace('#/+#', '/', $_SERVER[$key]);
        }
    }
    ...
    if (empty($_SERVER['REQUEST_URI']) 
        ...
        if (isset($_SERVER['HTTP_X_ORIGINAL_URL'])) {
            ...
            if (!isset($_SERVER['PATH_INFO']) && isset($_SERVER['ORIG_PATH_INFO'])) {
                $_SERVER['PATH_INFO'] = filter_var($_SERVER['ORIG_PATH_INFO'], FILTER_SANITIZE_URL);
            }

            // Construct REQUEST_URI safely
            ...
            if (!empty($_SERVER['QUERY_STRING'])) {
                $clean_query = preg_replace('/[^\p{L}\p{N}\-._~!$&\'()*+,;=:@\/?%]/', '', $_SERVER['QUERY_STRING']);
                $_SERVER['REQUEST_URI'] .= '?' . $clean_query;
            }
        }
    }
...
function wp_check_php_mysql_versions() {
	...
	if ( version_compare( $required_php_version, $php_version, '>' ) ) {
		...
		printf(
			'Your server is running PHP version %1$s but CosmicWord %2$s requires at least %3$s.',
			$php_version,
			$wp_version,
	...
	if ( ! function_exists( 'mysqli_connect' )
		...
		$message = '<p>' . __( 'Your PHP installation appears to be missing the MySQL extension which is required by CosmicWord.' ) . "</p>\n";
