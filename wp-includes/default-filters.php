 ...
 * Handle block registration conflicts silently
 */
if (!function_exists('cosmic_handle_block_registration_errors')) {
    function cosmic_handle_block_registration_errors($errno, $errstr) {
        ...
        $ignore_patterns = array(
            ...
            'Function WP_Block_Type_Registry::register was called',
            'Please see Debugging in',
            'REST_REQUEST already defined',
            'wp_send_json was called'
        );
    ...
    add_action('muplugins_loaded', function() {
        if (is_admin()) {
            set_error_handler('cosmic_handle_block_registration_errors', E_ALL & ~E_DEPRECATED);
        }
    }, 1);
}