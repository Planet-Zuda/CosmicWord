...
//	wp_add_dashboard_widget( 'dashboard_primary', __( 'CosmicWord Events and News' ), 'wp_dashboard_events_news' );
	...
	if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget_id'] ) ) {
		...
		wp_dashboard_trigger_widget_control( sanitize_key($_POST['widget_id']) );
		ob_end_clean();
...
function wp_add_dashboard_widget( $widget_id, $widget_name, $callback, $control_callback = null, $callback_args = null, $context = 'normal', $priority = 'core' ) {
	...
	if ( $control_callback && is_callable( $control_callback ) && current_user_can( 'edit_dashboard' ) ) {
		...
		if ( isset( $_GET['edit'] ) && $widget_id === $_GET['edit'] ) {
			list($url)    = explode( '#', esc_url(add_query_arg( 'edit', false ), 2 ));
			$widget_name .= ' <span class="postbox-title-action"><a href="' . esc_url( $url ) . '">' . __( 'Cancel' ) . '</a></span>';
			$callback     = '_wp_dashboard_control_callback';
		} else {
			list($url)    = explode( '#', esc_url(add_query_arg( 'edit', $widget_id ), 2 ));
 ...
 */
function wp_dashboard_control_callback( $dashboard, $meta_box ) {
    // Verify nonce first if this is a POST request
    if ( 'POST' === $_SERVER['REQUEST_METHOD'] ) {
        if ( 
            ! isset( $_POST['dashboard-widget-nonce'] ) ||
            ! wp_verify_nonce( 
                $_POST['dashboard-widget-nonce'], 
...
// function dashboard_control_callback fixed missing CSRF CosmicWord 1.1.2
/**
...
function wp_dashboard_rss_control( $widget_id, $form_inputs = array() ) {
    ...
    if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['widget-rss'][ $number ] ) ) {
        ...
        // Unslash and ensure array
        $rss_settings = wp_unslash( $_POST['widget-rss'][ $number ] );
        if (!is_array($rss_settings)) {
            wp_die(__('Invalid RSS widget settings format.'));
        }

        // Process settings through dedicated sanitization
        $widget_options[ $widget_id ] = wp_widget_rss_process($rss_settings);
        $widget_options[ $widget_id ]['number'] = $number;

        // Handle title if empty
        if ( empty($widget_options[ $widget_id ]['title']) && isset($rss_settings['title']) ) {
            // Sanitize URL before fetch
            $feed_url = esc_url_raw($widget_options[ $widget_id ]['url']);
            if (!empty($feed_url)) {
                $rss = fetch_feed($feed_url);
                if ( is_wp_error( $rss ) ) {
                    $widget_options[ $widget_id ]['title'] = esc_html__( 'Unknown Feed' );
                } else {
                    $widget_options[ $widget_id ]['title'] = esc_html(strip_tags( $rss->get_title() ));
                    $rss->__destruct();
                    unset( $rss );
                }
            }
        ...
        // Safely update options and clear cache
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			 ...
			 */
			'link'         => apply_filters( 'dashboard_primary_link', __( 'https://cosmicword.com/news/' ) ),
			 ...
			 */
			'url'          => apply_filters( 'dashboard_primary_feed', __( 'https://cosmicword.com/news/feed/' ) ),
			 ...
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'CosmicWord Blog' ) ),
			'items'        => 2,
			'show_summary' => 0,
			'show_author'  => 0,
			'show_date'    => 0,
		...
		'planet' => array(
			 ...
			 */
			'title'        => apply_filters( 'dashboard_secondary_title', __( 'Other CosmicWord News' ) ),
...
function wp_dashboard_browser_nag() {
    ...
    // Basic browser detection without external calls
    $browser_info = array(
        'name' => '',
        'insecure' => false
    );
    ...
    if ($is_IE || stripos($ua, 'MSIE') !== false || stripos($ua, 'Trident') !== false) {
        $msg = __('Internet Explorer does not give you the best CosmicWord experience. Switch to Microsoft Edge, or another more modern browser to get the most from your site.');
        $browser_info['name'] = 'Internet Explorer';
        $browser_info['insecure'] = true;
    } else {
        // No external version checks needed
        return;
    }

    // Build notice for IE only
    if ($browser_info['insecure']) {
        $notice .= sprintf(
            '<p class="browser-update-nag">%s</p>',
            esc_html($msg)
        );

        // Add Browse Happy link
        $browsehappy = 'https://browsehappy.com/';
        $locale = get_user_locale();
        if ('en_US' !== $locale) {
            $browsehappy = add_query_arg('locale', urlencode($locale), $browsehappy); // this is secured in $notice
        }
        ...
        // Add dismiss button
        $notice .= sprintf(
            '<p class="hide-if-no-js"><a href="" class="dismiss" aria-label="%1$s">%2$s</a></p>',
    ...
    /**
     * Filters the notice output for the browser warning.
     *
     * @param string $notice      The notice content
     * @param array  $browser_info Basic browser information
     */
    echo apply_filters('browser-warning-notice', $notice, $browser_info);
}
...
function wp_check_browser_version() {
    ...
    if (false === $response) {
        ...
        if (preg_match('/Firefox\/([0-9.]+)/', $ua, $matches)) {
            $response['name'] = 'Firefox';
            $response['version'] = sanitize_text_field($matches[1]);
        } elseif (preg_match('/Chrome\/([0-9.]+)/', $ua, $matches)) {
            $response['name'] = 'Chrome';
            $response['version'] = sanitize_text_field($matches[1]);
        } elseif (preg_match('/Safari\/([0-9.]+)/', $ua, $matches)) {
            $response['name'] = 'Safari';
            if (preg_match('/Version\/([0-9.]+)/', $ua, $ver_matches)) {
                $response['version'] = sanitize_text_field($ver_matches[1]);
            }
        ...
        // Set current version as detected version since we're not checking online
        $response['current_version'] = $response['version'];
...
function wp_dashboard_php_nag() {
	...
	<p>
		<?php _e( 'PHP is one of the programming languages used to build CosmicWord. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.' ); ?>
		<?php
...
function wp_welcome_panel() {
	list( $display_version ) = explode( '-', COSMIC_VERSION);
	...
	<div class="welcome-panel-header">
		...
		<h2><?php _e( 'Welcome to CosmicWord!' ); ?></h2>
		<p>
	...
	<div class="welcome-panel-column-container">
		...
		<div class="welcome-panel-column">
			...
			<?php else : ?>
				...
				<p><?php _e( 'There is a new kind of CosmicWord theme, called a block theme, that lets you build the site you&#8217;ve always wanted &#8212; with blocks and styles.' ); ?></p>
