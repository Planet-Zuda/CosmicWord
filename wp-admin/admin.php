...
if ( isset( $_REQUEST['taxonomy'] ) && taxonomy_exists( $_REQUEST['taxonomy'] ) ) {
	$taxnow = sanitize_text_field($_REQUEST['taxonomy']);
} else {
	$taxnow = '';
}
...
if ( empty( $page_hook ) && 'edit.php' === $pagenow && get_plugin_page_hook( $plugin_page, 'tools.php' ) ) {
    ...
    if ( ! empty( $_SERVER['QUERY_STRING'] ) ) {
        ...
        // Ensure 'page' parameter is from plugin_page if not in query
        if (!isset($filtered_params['page']) && $plugin_page) {
            $filtered_params['page'] = sanitize_key($plugin_page);
        }
        ...
        // Just add the page parameter
        $redirect_url .= '?page=' . sanitize_key($plugin_page);
    }
    
    // Verify nonce if this is an action
    if (!empty($filtered_params['action'])) {
        check_admin_referer($filtered_params['action']);
    }
